...
module.exports = function(grunt) {
    ...
    grunt.registerTask('open', 'Opens the application with a browser', function() {
        ...
        const COMMAND = isWindows ? "start" : "sensible-browser";
        const URL = "http://localhost:8700/app";
        const child = spawn(COMMAND, [URL]);
        child.stdout.setEncoding('utf8'); // use if you want text chunks
        child.stdout.on('data', function (chunk) {
            console.log("Passing data: " + chunk);
        });
        // since these are streams, you can pipe them elsewhere
        child.stderr.pipe(function(chunk) {
